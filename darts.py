...
async def place_dart_bet(message: types.Message, state: FSMContext):
    ...
    if len(message.text.split()) < 2:
        await message.reply(
            "🎯 <b>Darts</b>\n\n"
            "To play, type the command /darts with the desired bet amount.\n\n"
            "<b>Example:</b>\n"
            "/darts 10.50 - to play for $10.50\n"
            "/darts half - to play for half of your balance\n"
            "/darts all - to play all-in",
            parse_mode="HTML"
        )
        return
    ...
    best_of_3_button = InlineKeyboardButton(text="First to 3 points", callback_data=f"select_dart_rounds:best_of_3:{user_id}")
    best_of_2_button = InlineKeyboardButton(text="First to 2 points", callback_data=f"select_dart_rounds:best_of_2:{user_id}")
    best_of_1_button = InlineKeyboardButton(text="First to 1 point", callback_data=f"select_dart_rounds:best_of_1:{user_id}")
    cancel_button = InlineKeyboardButton(text="❌ Cancel", callback_data=f"cancel_dart_bet:{user_id}")
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("select_dart_rounds"))
async def select_dart_rounds(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    accept_button = InlineKeyboardButton(text="✅ Accept Bet", callback_data=f"accept_dart_bet:{bettor_id}")
    cancel_button = InlineKeyboardButton(text="❌ Cancel Bet", callback_data=f"cancel_dart_bet:{bettor_id}")

    keyboard.add(accept_button)
    if 1 <= bet_data.amount <= 100:
        play_vs_bot_button = InlineKeyboardButton(text="🤖 Play vs Dealer", callback_data=f"play_vs_dartbot:{bettor_id}")
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("play_vs_dartbot"))
async def play_vs_dartbot(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    save_current_game(game_id, chat_id, bettor_id, DEALER_ID, bet_data.amount, game_mode, bet_data.required_wins, bettor_id, rolls, round_number, bettor_wins, opponent_wins, bet_mode="darts")
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("accept_dart_bet"))
async def accept_dart_bet(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Save the game in the database
    save_current_game(game_id, chat_id, bettor_id, acceptor_id, bet_data.amount, game_mode, bet_data.required_wins, bettor_id, rolls, round_number, bettor_wins, opponent_wins, bet_mode="darts")
    ...
    await bot.send_message(
        bettor_id,
        f"🎯 Your darts game has been [accepted]({message_link}) by {callback_query.from_user.first_name}!",
        parse_mode="Markdown",
    )

@dp.callback_query_handler(lambda c: c.data and c.data.startswith("cancel_dart_bet"))
async def cancel_dart_bet(callback_query: types.CallbackQuery):
...
async def bot_throw(chat_id, game_id):
    ...
    await proceed_to_next_turn_or_complete_dart_round(None, chat_id, game_id)

async def handle_dart_throw(message: types.Message):
    logging.info(f"Received dart throw from user: {message.from_user.id}")
    ...
    if str(user_id) in rolls:
        await message.reply("❗ You've already thrown! Wait for your opponent's turn.", parse_mode="HTML")
        return

    roll_value = message.dice.value
    logging.info(f"User {user_id} threw a dart with value {roll_value}")
    ...
    if game.opponent == DEALER_ID:
        ...
        await message.reply("🤖 <b>Dealer Bot's turn!</b> Dealer is throwing...", parse_mode="HTML")
        await bot_throw(chat_id, game.game_id)
    else:
        await proceed_to_next_turn_or_complete_dart_round(message, chat_id, game.game_id)

async def proceed_to_next_turn_or_complete_dart_round(message: types.Message, chat_id: str, game_id: str):
    game = CurrentGame.get_or_none((CurrentGame.chat_id == chat_id) & (CurrentGame.game_id == game_id))
    if not game:
        logging.info(f"No game found with game_id: {game_id}")
        return
    ...
    if len(rolls) == 2:
        await complete_dart_round(message, chat_id, game_id)
    else:
        if game.opponent != DEALER_ID:
...
async def complete_dart_round(message: types.Message, chat_id: str, game_id: str):
    game = CurrentGame.get_or_none((CurrentGame.chat_id == chat_id) & (CurrentGame.game_id == game_id))
    if not game:
        logging.info(f"No game found with game_id: {game_id}")
        return
    ...
    if player1_roll in [1, 2] or player2_roll in [1, 2]:
        await bot.send_message(
            chat_id,
            f"<b>Score:</b>\n"
    ...
    if game.bettor_wins >= required_wins:
        await determine_dart_winner(message, chat_id, game_id, winner_id=player1_id)
    elif game.opponent_wins >= required_wins:
        await determine_dart_winner(message, chat_id, game_id, winner_id=player2_id)
    else:
...
async def determine_dart_winner(message: types.Message, chat_id: str, game_id: str, winner_id=None):
    game = CurrentGame.get_or_none((CurrentGame.chat_id == chat_id) & (CurrentGame.game_id == game_id))
    if not game:
        logging.info(f"No game found with game_id: {game_id}")
        return
    ...
    if winner_id != DEALER_ID:
        congrats_message = f"🥇<b>{winner_name}</b>! Won <b>${winner_earnings:,.2f} in Darts 🎯</b>!"
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(handle_dart_throw, content_types=types.ContentType.DICE)
