...
from db.database import (
    ...
    delete_current_game, get_current_games
)
...
async def place_basket_bet(message: types.Message, state: FSMContext):
    ...
    if CurrentGame.select().where(CurrentGame.chat_id == chat_id, (CurrentGame.bettor == user_id) | (CurrentGame.opponent == user_id)).exists():
        await message.reply("ℹ️ <b>You are already in an ongoing game!</b>", parse_mode="HTML")
        return

    if len(message.text.split()) < 2:
        await message.reply(
            "🏀 <b>Basketball</b>\n\n"
            "To play, type the command /bask with the desired bet amount.\n\n"
            "<b>Example:</b>\n"
            "/bask 10.50 - to play for $10.50\n"
            "/bask half - to play for half of your balance\n"
            "/bask all - to play all-in",
            parse_mode="HTML"
        )
        return
    ...
    best_of_3_button = InlineKeyboardButton(text="First to 3 points", callback_data=f"select_basket_rounds:best_of_3:{user_id}")
    best_of_2_button = InlineKeyboardButton(text="First to 2 points", callback_data=f"select_basket_rounds:best_of_2:{user_id}")
    best_of_1_button = InlineKeyboardButton(text="First to 1 point", callback_data=f"select_basket_rounds:best_of_1:{user_id}")
    cancel_button = InlineKeyboardButton(text="❌ Cancel", callback_data=f"cancel_basket_bet:{user_id}")
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("select_basket_rounds"))
async def select_basket_rounds(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    accept_button = InlineKeyboardButton(text="✅ Accept Bet", callback_data=f"accept_basket_bet:{bettor_id}")
    cancel_button = InlineKeyboardButton(text="❌ Cancel Bet", callback_data=f"cancel_basket_bet:{bettor_id}")

    keyboard.add(accept_button)

    if 1 <= bet_data.amount <= 100:
        play_vs_bot_button = InlineKeyboardButton(text="🤖 Play vs Dealer", callback_data=f"play_vs_basketbot:{bettor_id}")
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("play_vs_basketbot"))
async def play_vs_basketbot(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    save_current_game(game_id, chat_id, bettor_id, DEALER_ID, bet_data.amount, game_mode, bet_data.required_wins, bettor_id, rolls, round_number, bettor_wins, opponent_wins, bet_mode="bask")
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("accept_basket_bet"))
async def accept_basket_bet(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    game_id = str(uuid.uuid4())
    save_current_game(chat_id, game_id, bettor_id, acceptor_id, bet_data.amount, bet_data.game_mode, bet_data.required_wins, bettor_id, bet_mode="bask")
    ...
    await bot.send_message(
        bettor_id,
        f"🏀 Your basketball game has been [accepted]({message_link}) by {callback_query.from_user.first_name}!",
        parse_mode="Markdown",
    )

async def bot_shot(chat_id, game_id):
    ...
    # Store dealer's roll in the database
    ...
    await proceed_to_next_turn_or_complete_basket_round(None, chat_id, game_id)

async def handle_basketball_shot(message: types.Message):
    logging.info(f"Received basketball roll from user: {message.from_user.id}")
    ...
    if str(user_id) in rolls:
        await message.reply("❗ You've already taken your shot! Wait for your opponent's turn.", parse_mode="HTML")
        return
    ...
    if game.opponent == DEALER_ID:
        ...
        await bot_shot(chat_id, game.game_id)
    else:
        # If it's a player vs player match, switch turns to the opponent
        await proceed_to_next_turn_or_complete_basket_round(message, chat_id, game.game_id)

async def proceed_to_next_turn_or_complete_basket_round(message: types.Message, chat_id: str, game_id: str):
    # Fetch the game from the database
    ...
    if len(rolls) == 2:
        ...
        await complete_basket_round(message, chat_id, game_id)
    else:
...
async def complete_basket_round(message: types.Message, chat_id: str, game_id: str):
    # Fetch the game from the database
    game = CurrentGame.get_or_none(CurrentGame.chat_id == chat_id, CurrentGame.game_id == game_id)
    ...
    if player2_id == DEALER_ID:
        if player1_roll == player2_roll:
            logging.warning(f"Tie detected with both players rolling a {player1_roll}. Rolling again.")
            ...
            game.bettor_wins += 1
        elif player1_roll in [4] and player2_roll in [1, 2, 3]:
            game.bettor_wins += 1
    else:
        if player1_roll > player2_roll:
            game.bettor_wins += 1
    ...
    if game.bettor_wins >= required_wins:
        await determine_basket_winner(message, chat_id, game_id, winner_id=player1_id)
    elif game.opponent_wins >= required_wins:
        await determine_basket_winner(message, chat_id, game_id, winner_id=player2_id)
    else:
...
async def determine_basket_winner(message: types.Message, chat_id: str, game_id: str, winner_id=None):
    # Fetch the game from the database
    ...
    # Forward the congratulations message to another channel if the winner is not the dealer
    if winner_id != DEALER_ID:
        congrats_message = f"🥇<b>{winner_name}</b>! Won <b>${winner_earnings:,.2f} in Basketball 🏀</b>!"
    ...
    # Record match in the database and clean up game data
    Match.create(
        chat_id=chat_id,
        game_id=game_id,
        bettor=player1_id,
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("cancel_basket_bet"))
async def cancel_basket_bet(callback_query: types.CallbackQuery):
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(handle_basketball_shot, content_types=types.ContentType.DICE)
